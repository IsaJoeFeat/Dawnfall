extends Node


const WORLD_SIZE := Vector2(
	192.0,
	192.0
)

const CELL_SIZE: float = 4.0

const WALL_COLUMN: int = 24
const GAP_START_ROW: int = 22
const GAP_END_ROW: int = 24

const UNIT_COUNT: int = 32
const FORMATION_COLUMNS: int = 8
const FORMATION_ROWS: int = 4
const FORMATION_SPACING: float = 2.0

const UNIT_SPEED: float = 20.0
const MAXIMUM_TEST_TICKS: int = 1200


func _ready() -> void:
	var world := SimulationWorld.new()

	world.configure_navigation(
		Vector3.ZERO,
		WORLD_SIZE,
		CELL_SIZE
	)

	var navigation_grid: NavigationGrid = (
		world.navigation_grid
	)

	_build_wall(
		world,
		navigation_grid
	)

	var start_center := Vector3(
		48.0,
		0.0,
		40.0
	)

	var destination_center := Vector3(
		144.0,
		0.0,
		40.0
	)

	assert(
		not navigation_grid.is_segment_traversable(
			start_center,
			destination_center
		),
		"Direct group-center path should be blocked."
	)

	var expected_route: PackedVector3Array = (
		world.build_shared_route(
			start_center,
			destination_center
		)
	)

	assert(
		expected_route.size() > 2,
		"Group should require an indirect shared route."
	)

	for waypoint_index: int in range(
		1,
		expected_route.size()
	):
		assert(
			navigation_grid.is_segment_traversable(
				expected_route[
					waypoint_index - 1
				],
				expected_route[
					waypoint_index
				]
			),
			"Every shared route segment must be terrain-legal."
		)

	var entity_ids := PackedInt64Array()
	var destinations := PackedVector3Array()
	var previous_positions := PackedVector3Array()

	var crossed_gap := PackedByteArray()
	crossed_gap.resize(UNIT_COUNT)
	crossed_gap.fill(0)

	for unit_index: int in range(
		UNIT_COUNT
	):
		var formation_offset: Vector3 = (
			_get_formation_offset(
				unit_index
			)
		)

		var spawn_position: Vector3 = (
			start_center
			+ formation_offset
		)

		var final_destination: Vector3 = (
			destination_center
			+ formation_offset
		)

		assert(
			navigation_grid.is_world_position_traversable(
				spawn_position
			),
			"Every unit must begin on traversable terrain."
		)

		assert(
			navigation_grid.is_world_position_traversable(
				final_destination
			),
			"Every final slot must be traversable."
		)

		var entity_id: int = (
			world.entities.create_entity(
				0,
				0,
				spawn_position,
				100.0,
				0.35,
				UNIT_SPEED,
				100.0,
				100.0,
				deg_to_rad(720.0),
				0.0
			)
		)

		assert(
			EntityId.is_valid(
				entity_id
			),
			"Elastic-group test unit should spawn."
		)

		entity_ids.append(
			entity_id
		)

		destinations.append(
			final_destination
		)

		previous_positions.append(
			spawn_position
		)

	world.rebuild_spatial_grid()

	var accepted_count: int = (
		world.issue_group_move(
			entity_ids,
			destinations
		)
	)

	assert(
		accepted_count == UNIT_COUNT,
		"Every unit should accept the group move."
	)

	assert(
		world.shared_route_request_count == 1,
		"One group command should generate exactly one shared route."
	)

	assert(
		world.last_shared_route.size() > 2,
		"Issued command should use an indirect shared route."
	)

	var wall_crossing_x: float = (
		float(WALL_COLUMN)
		* CELL_SIZE
	)

	var completed_ticks: int = 0
	var maximum_neighbors: int = 0

	var simulation_start: int = (
		Time.get_ticks_usec()
	)

	for tick_index: int in range(
		MAXIMUM_TEST_TICKS
	):
		world.advance(
			world.clock.tick_seconds
		)

		completed_ticks = (
			tick_index + 1
		)

		maximum_neighbors = maxi(
			maximum_neighbors,
			world.movement_system.last_maximum_neighbors
		)

		var unfinished_count: int = 0

		for unit_index: int in range(
			UNIT_COUNT
		):
			var entity_id: int = (
				entity_ids[
					unit_index
				]
			)

			var entity_index: int = (
				world.entities.get_index_if_alive(
					entity_id
				)
			)

			assert(
				entity_index >= 0,
				"Every navigation-test unit should remain alive."
			)

			var current_position: Vector3 = (
				world.entities.positions[
					entity_index
				]
			)

			assert(
				navigation_grid.is_world_position_traversable(
					current_position
				),
				"Unit entered Impassable terrain."
			)

			if (
				crossed_gap[
					unit_index
				] == 0
			):
				_check_gap_crossing(
					previous_positions[
						unit_index
					],
					current_position,
					wall_crossing_x,
					crossed_gap,
					unit_index
				)

			previous_positions[
				unit_index
			] = current_position

			if world.entities.has_move_target_by_index(
				entity_index
			):
				unfinished_count += 1

		if unfinished_count == 0:
			break

	var elapsed_milliseconds: float = (
		float(
			Time.get_ticks_usec()
			- simulation_start
		)
		/ 1000.0
	)

	var unfinished_units: int = 0
	var units_missing_gap: int = 0
	var maximum_final_error: float = 0.0

	for unit_index: int in range(
		UNIT_COUNT
	):
		var entity_index: int = (
			world.entities.get_index_if_alive(
				entity_ids[
					unit_index
				]
			)
		)

		if world.entities.has_move_target_by_index(
			entity_index
		):
			unfinished_units += 1

		if crossed_gap[
			unit_index
		] == 0:
			units_missing_gap += 1

		var final_error: float = (
			world.entities.positions[
				entity_index
			].distance_to(
				destinations[
					unit_index
				]
			)
		)

		maximum_final_error = maxf(
			maximum_final_error,
			final_error
		)

	if (
		unfinished_units > 0
		or units_missing_gap > 0
		or maximum_final_error > 0.01
	):
		_print_failure_diagnostics(
			world,
			entity_ids,
			destinations,
			crossed_gap
		)

	assert(
		units_missing_gap == 0,
		"Every unit should cross through the navigation gap."
	)

	assert(
		unfinished_units == 0,
		"Every unit should complete its routed move."
	)

	assert(
		maximum_final_error <= 0.01,
		"Every unit should reform at its assigned final destination."
	)

	print(
		(
			"ELASTIC GROUP NAVIGATION PASS | "
			+ "%d units | "
			+ "%d shared waypoints | "
			+ "%d ticks | "
			+ "%.2f ms simulation | "
			+ "max neighbors %d | "
			+ "max final error %.3f"
		)
		% [
			UNIT_COUNT,
			world.last_shared_route.size(),
			completed_ticks,
			elapsed_milliseconds,
			maximum_neighbors,
			maximum_final_error,
		]
	)


func _build_wall(
	world: SimulationWorld,
	navigation_grid: NavigationGrid
) -> void:
	for row: int in range(
		navigation_grid.row_count()
	):
		if (
			row >= GAP_START_ROW
			and row <= GAP_END_ROW
		):
			continue

		world.set_navigation_cell_blocked(
			Vector2i(
				WALL_COLUMN,
				row
			)
		)


func _get_formation_offset(
	unit_index: int
) -> Vector3:
	var column: int = (
		unit_index
		% FORMATION_COLUMNS
	)

	var row: int = (
		unit_index
		/ FORMATION_COLUMNS
	)

	var z_offset: float = (
		(
			float(column)
			- float(
				FORMATION_COLUMNS - 1
			) * 0.5
		)
		* FORMATION_SPACING
	)

	var x_offset: float = (
		(
			float(row)
			- float(
				FORMATION_ROWS - 1
			) * 0.5
		)
		* FORMATION_SPACING
	)

	return Vector3(
		x_offset,
		0.0,
		z_offset
	)


func _check_gap_crossing(
	previous_position: Vector3,
	current_position: Vector3,
	wall_crossing_x: float,
	crossed_gap: PackedByteArray,
	unit_index: int
) -> void:
	if (
		previous_position.x
		>= wall_crossing_x
		or current_position.x
		< wall_crossing_x
	):
		return

	var x_difference: float = (
		current_position.x
		- previous_position.x
	)

	if is_zero_approx(
		x_difference
	):
		return

	var interpolation: float = (
		(
			wall_crossing_x
			- previous_position.x
		)
		/ x_difference
	)

	var crossing_z: float = lerpf(
		previous_position.z,
		current_position.z,
		interpolation
	)

	var crossing_row: int = floori(
		crossing_z
		/ CELL_SIZE
	)

	assert(
		crossing_row >= GAP_START_ROW
		and crossing_row <= GAP_END_ROW,
		"Unit crossed the wall outside the legal gap."
	)

	crossed_gap[
		unit_index
	] = 1


func _print_failure_diagnostics(
	world: SimulationWorld,
	entity_ids: PackedInt64Array,
	destinations: PackedVector3Array,
	crossed_gap: PackedByteArray
) -> void:
	print("----- ELASTIC GROUP DEBUG -----")
	print(
		"Route: ",
		world.last_shared_route
	)

	var printed_count: int = 0

	for unit_index: int in range(
		UNIT_COUNT
	):
		var entity_index: int = (
			world.entities.get_index_if_alive(
				entity_ids[
					unit_index
				]
			)
		)

		var unfinished: bool = (
			world.entities.has_move_target_by_index(
				entity_index
			)
		)

		var final_error: float = (
			world.entities.positions[
				entity_index
			].distance_to(
				destinations[
					unit_index
				]
			)
		)

		if (
			not unfinished
			and crossed_gap[
				unit_index
			] == 1
			and final_error <= 0.01
		):
			continue

		print(
			(
				"Unit %d | "
				+ "pos %s | "
				+ "target %s | "
				+ "moving %s | "
				+ "crossed gap %s | "
				+ "final error %.3f"
			)
			% [
				unit_index,
				str(
					world.entities.positions[
						entity_index
					]
				),
				str(
					world.entities.movement_targets[
						entity_index
					]
				),
				str(unfinished),
				str(
					crossed_gap[
						unit_index
					] == 1
				),
				final_error,
			]
		)

		printed_count += 1

		if printed_count >= 8:
			break

	print("-------------------------------")
